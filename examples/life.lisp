...
(defsketch life
    ((title "Conway's Game of Life")
     ...
     (running nil))
  (sketch-utils:with-fit ((* columns cell-size) (* rows cell-size) width height)
...
(defmethod kit.sdl2:mousebutton-event ((window life) state ts b x y)
  (when (eq state :mousebuttondown)
    (with-slots (cells front running cell-size columns rows width height) window
      (destructuring-bind (x y)
          (sketch-utils:fit-point x y (* columns cell-size) (* rows cell-size) width height)
        (when (not running)
	  (let ((cy (1+ (truncate (/ y cell-size))))
	        (cx (1+ (truncate (/ x cell-size)))))
            (when (and (< 0 cx (1+ columns))
                       (< 0 cy (1+ rows)))
